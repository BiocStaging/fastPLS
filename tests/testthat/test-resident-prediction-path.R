...
test_that("resident Metal predicts a complete component path consistently", {
    ...
    path <- c(1L, 3L, 5L)
    fit <- pls(
        X, y, ncomp = path, method = "simpls",
        backend = "metal", fit = FALSE, return_variance = FALSE,
        seed = 12
    )
    object <- fastPLS:::.fastpls_restore_internal_output_fields(fit)
    bits <- fastPLS:::.resident_metal_input(
        X[seq_len(25), ], "newdata"
    )
    actual <- fastPLS:::metal_resident_predict_path_cpp(
        object$resident_state, bits, path, 0L
    )

    for (index in seq_along(path)) {
        expected <- fastPLS:::metal_resident_predict_cpp(
            object$resident_state, bits, path[[index]], 0L
        )
        expect_equal(
            fastPLS:::.resident_metal_summary(actual[, , index]),
            fastPLS:::.resident_metal_summary(expected),
            tolerance = 2e-6
        )
    }
})

test_that("resident Metal reuses one projection for CV labels and responses", {
    skip_if_not(fastPLS::has_metal())

    set.seed(725)
    X <- float::fl(matrix(rnorm(220 * 28), 220, 28))
    y <- factor(rep(letters[1:5], length.out = nrow(X)))
    path <- c(1L, 2L, 4L)
    fit <- pls(
        X, y, ncomp = path, method = "simpls",
        backend = "metal", classifier = "lda", fit = FALSE,
        return_variance = FALSE, seed = 17
    )
    object <- fastPLS:::.fastpls_restore_internal_output_fields(fit)
    bits <- fastPLS:::.resident_metal_input(X[seq_len(37), ], "newdata")
    combined <- fastPLS:::metal_resident_classify_response_path_cpp(
        object$resident_state, bits, path, 1L, 1L
    )
    labels <- fastPLS:::metal_resident_classify_path_cpp(
        object$resident_state, bits, path, 1L, 1L
    )
    responses <- fastPLS:::metal_resident_predict_path_cpp(
        object$resident_state, bits, path, 0L
    )
    expect_identical(combined$labels, labels)
    for (index in seq_along(path)) {
        expect_equal(
            fastPLS:::.resident_metal_summary(
                combined$predictions[, , index]
            ),
            fastPLS:::.resident_metal_summary(responses[, , index]),
            tolerance = 2e-6
        )
    }
})
