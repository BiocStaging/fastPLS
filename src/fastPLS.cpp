...
arma::fmat core_float_matrix(
    const fastpls::core::Matrix<float>& source) {
  return arma::fmat(
    source.data(), static_cast<arma::uword>(source.rows()),
    static_cast<arma::uword>(source.columns())
  );
}

Rcpp::List fit_simpls_float32_labels_core(
    arma::fmat Xtrain,
    const Rcpp::IntegerVector& labels,
    int n_classes,
  ...
  const int p = static_cast<int>(Xtrain.n_cols);
  const int max_ncomp = arma::max(ncomp);
  arma::frowvec x_mean(p, arma::fill::zeros);
  if (scaling < 3) {
    x_mean = arma::mean(Xtrain, 0);
    Xtrain.each_row() -= x_mean;
  }
  arma::frowvec x_scale(p, arma::fill::ones);
  if (scaling == 2) {
    x_scale = float_col_sd(Xtrain);
    Xtrain.each_row() /= x_scale;
  }
  const Float32LabelResponse response =
    label_response_float32(Xtrain, labels, n_classes);

  fastpls::core::SimplsControls controls;
  controls.components = static_cast<std::size_t>(max_ncomp);
  controls.maximum_block = static_cast<std::size_t>(
    fastpls::native::candidate_block_size(
      max_ncomp, p, n_classes, true, n, kCpuSimplsBlockSize
    )
  );
  const int minimum_components = env_int_or(
    "FASTPLS_FAST_CROSSPROD_MIN_NCOMP", 20, 1, 1024
  );
  const int maximum_predictors = env_int_or(
    "FASTPLS_FAST_CROSSPROD_MAX_P", kCpuCrossprodMaxPredictors,
    16, 65536
  );
  const int minimum_ratio = env_int_or(
    "FASTPLS_FAST_CROSSPROD_MIN_N_TO_P_RATIO", 8, 1, 1024
  );
  controls.cache_predictor_crossprod =
    max_ncomp >= minimum_components && p <= n &&
    n >= p * minimum_ratio && p <= maximum_predictors;
  ...
  controls.rsvd.oversample = rsvd_oversample;
  controls.rsvd.power = rsvd_power;
  controls.rsvd.seed = static_cast<unsigned int>(seed);
  ...
  const auto model = fastpls::core::fit_simpls_preprocessed<float>(
    ...
    fastpls::core::make_const_view(
      response.crosscov.memptr(), response.crosscov.n_rows,
      response.crosscov.n_cols, response.crosscov.n_rows
    ),
    controls, backend, workspace
  );
  if (model.completed_components <
      static_cast<std::size_t>(max_ncomp)) {
    Rcpp::stop(
      "float32 core SIMPLS returned fewer components than requested"
    );
  }
  arma::fvec r2(ncomp.n_elem, arma::fill::value(NA_REAL));
  Rcpp::List output = Rcpp::List::create(
    Rcpp::Named("P") = R_NilValue,
    Rcpp::Named("R") = fmat_to_float32_bits(
      core_float_matrix(model.weights)
    ),
    Rcpp::Named("Q") = fmat_to_float32_bits(
      core_float_matrix(model.response_loadings)
    ),
    Rcpp::Named("Ttrain") = fmat_to_float32_bits(
      core_float_matrix(model.scores)
    ),
    Rcpp::Named("mX") = fmat_to_float32_bits(arma::fmat(x_mean)),
    Rcpp::Named("vX") = fmat_to_float32_bits(arma::fmat(x_scale)),
    Rcpp::Named("mY") = fmat_to_float32_bits(arma::fmat(response.mean)),
    Rcpp::Named("p") = p,
    Rcpp::Named("m") = n_classes,
    Rcpp::Named("ncomp") = ncomp,
    Rcpp::Named("Yfit") = R_NilValue,
    Rcpp::Named("R2Y") = r2,
    Rcpp::Named("pls_method") = "simpls",
    Rcpp::Named("xprod_mode") = "float32_label_class_sums_blocked"
  );
  if (phase_timing) {
    output["benchmark_phase_timing"] = Rcpp::List::create(
      Rcpp::Named("preprocess_crosscov_sec") = model.timing.setup,
      Rcpp::Named("response_crosscov_sec") = 0.0,
      Rcpp::Named("crossprod_cache_sec") = 0.0,
      Rcpp::Named("right_gram_sec") = 0.0,
      Rcpp::Named("estimator_sec") = model.timing.total,
      Rcpp::Named("direction_sec") = model.timing.direction,
      Rcpp::Named("component_update_sec") =
        model.timing.component_updates,
      Rcpp::Named("coefficient_path_sec") = 0.0,
      Rcpp::Named("fitted_values_sec") = 0.0,
      Rcpp::Named("candidate_geometry_sec") =
        model.timing.candidate_geometry,
      Rcpp::Named("cpp_total_sec") = model.timing.total
    );
  }
  return output;
}
...
Rcpp::List pls_float32_labels_cpp(
  ...
  if (method != 1) {
    ...
    if (backend == 0 && !fit) {
      return fit_simpls_float32_labels_core(
        std::move(Xtrain), labels, n_classes, ncomp, scaling,
        rsvd_oversample, rsvd_power, seed, phase_timing
      );
    }
    fastpls::native::SimplsOptions controls;
    controls.scaling = scaling;
